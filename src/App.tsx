...
export default function App() {
  ...
  const renderSetup = () => (
    <div className="max-w-3xl mx-auto bg-white rounded-[2.5rem] shadow-[0_20px_50px_-12px_rgba(30,58,138,0.15)] overflow-hidden border border-slate-100">
      ...
      <div className="flex border-b border-slate-100 bg-slate-50/50 p-1.5 gap-1.5 relative">
        <button 
        ...
        <button 
          ...
          className={`px-3 rounded-xl transition-all flex items-center justify-center border ${isApiKeyPanelOpen ? 'bg-amber-100 border-amber-200 text-amber-600' : 'bg-slate-100 border-slate-200 text-slate-400'}`}
      ...
      {/* API Key Panel - Small & Hideable */}
      {isApiKeyPanelOpen && (
        <div className="px-6 py-4 bg-amber-50/50 border-b border-amber-100 animate-in slide-in-from-top duration-300">
          <div className="max-w-md mx-auto space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-black text-amber-600 uppercase tracking-widest flex items-center gap-1.5">
                <Zap className="w-3 h-3" /> Gemini API Key (Opsional)
              </label>
              <span className="text-[9px] text-amber-500/60 font-medium italic">Gunakan jika kuota gratis habis</span>
            </div>
            <div className="relative flex items-center gap-2">
              <div className="relative flex-1">
                <input 
                  ...
                  className="w-full pl-3 pr-10 py-2 bg-white border border-amber-200 rounded-xl text-xs font-mono focus:ring-2 focus:ring-amber-400 focus:border-transparent outline-none transition-all shadow-sm"
                />
                <button 
                  type="button"
              ...
              <button 
                ...
                className="p-2 bg-white border border-amber-200 text-amber-500 rounded-xl hover:bg-amber-100 transition-all shadow-sm"
                title="Salin API Key"
              >
